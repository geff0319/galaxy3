...
export const FfmpegWorkDirectory = 'data/ffmpeg'
export const YtDlpWorkDirectory = 'data/yt-dlp'
...
export const getFfmpegFileName = async () => {
  const envStore = useEnvStore()
  const { os, arch } = await GetEnv()
  const fileSuffix = { windows: '.exe', linux: '', darwin: '' }[os]
  return `ffmpeg${fileSuffix}`
}

export const getYtDlpFileName = async () => {
  const envStore = useEnvStore()
  const { os, arch } = await GetEnv()
  const fileSuffix = { windows: '.exe', linux: '', darwin: '' }[os]
  return `yt-dlp${fileSuffix}`
}
